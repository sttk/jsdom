...
function setChild(core, parentImpl, node) {
  ...
  const shouldLoadContentBeforeAddingToDom =
    newNode instanceof HTMLScriptElementImpl ||
    newNode instanceof HTMLStyleElementImpl;

  if (shouldLoadContentBeforeAddingToDom && node.children) {
    for (let c = 0; c < node.children.length; c++) {
      setChild(core, newNode, node.children[c]);
    }
  }

  if (newNode instanceof HTMLScriptElementImpl) {
    newNode._nonBlocking = false;
  }
