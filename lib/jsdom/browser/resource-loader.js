...
const syncWorker = require.resolve("./resource-load-sync");
...
function createResourceLoadHandler(element, resourceUrl, document, loadCallback) {
  ...
  return (err, data, response) => {
    ...
    if (err) {
      if (!err.isAbortError) {
        exports.dispatchErrorEvent(element, err, resourceUrl);
      }
    }
  };
}
...
function fetch(urlObj, options, callback) {
  ...
  const filePath = exports.urlToPath(urlObj.pathname);
...
exports.loadAsync = function (element, urlString, options) {
  function then(resolve) {
    exports.load(element, urlString, options, (text, filename) => {
...
exports.loadSync = function (element, urlString, options) {
  const document = element._ownerDocument;
  const documentImpl = document.implementation;

  const tagName = element.tagName.toLowerCase();
  if (!documentImpl._hasFeature("FetchExternalResources", tagName)) {
    return { then: noop };
  }
  ...
  const fetchFn = urlObj.hostname ? fetchSync : readSync;
  ...
  if (typeof customLoader === "function") {
    then = resolve => {
      customLoader({
        ...
        baseUrl: documentBaseURLSerialized(document),
        defaultFetch(fetchCallback) {
          fetchCallback(element, urlString, options, resolve);
        }
      }, fetchFn);
    };
  } else {
    then = resolve => {
      fetchFn(element, urlString, options, resolve);
    };
  }
  return { then };
};

function readSync(element, url, options, resolve) {
  const { defaultEncoding, detectMetaCharset } = options;

  const data = fs.readFileSync(exports.urlToPath(url));
...
function fetchSync(element, url, options, resolve) {
  const input = JSON.stringify({ url, options });

  const resp = spawnSync(process.execPath, [syncWorker], { input });

  let err = resp.error;
  if (!err) {
    const output = JSON.parse(resp.stdout.toString(), (k, v) => {
    ...
    if (!err) {
      try {
        resolve(output.data, output.response);
  ...
  if (err && !err.isAbortError) {
    exports.dispatchErrorEvent(element, err, url);
  }
}
...
exports.dispatchErrorEvent = function (element, err, url) {
  const document = element._ownerDocument;
  ...
  const error = new Error(`Could not load ${element.localName}: "${url}"`);
  error.detail = err;
